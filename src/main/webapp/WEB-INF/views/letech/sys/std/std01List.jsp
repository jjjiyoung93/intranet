...
<body>
	<div id="warpper">
		...
		<div id="page-wrapper">
			<section class="row">
				<div class="col-lg-10">
			<form name="form1" id="form1" method="post" action="<%=request.getContextPath()%>/sys/std/std01List.do" >
...
function fnInsert(){
	...
	window.open('std01Form.do?flag=1','target_name','scrollbars=yes,toolbar=yes,resizable=yes,width=500,height=450,left=0,top=0');
...
function fnModify(cd){
	...
	window.open('std01Form.do?flag=3&aut_cd='+cd,'target_name','scrollbars=yes,toolbar=yes,resizable=yes,width=500,height=300,left=0,top=0');
}
...
function fnDelete(mode,cd){
	...
	if(confirm('삭제는 신중히 확인 후 하세요. \r\n 삭제하시겠습니까?')){
		document.form1.action = "<%=request.getContextPath() %>/sys/std/std01Tran.do";
		document.form1.submit();
	}
}
