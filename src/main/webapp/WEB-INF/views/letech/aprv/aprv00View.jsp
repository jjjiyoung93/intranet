...
<c:if test="${viewMap.REPT_APRV_NO eq loginVO.id && viewMap.APRV_LINE_CD eq '0' && viewMap.CONF_TYPE eq 'N' }">
				    <input class="btn btn_info" type="button" id="btn-ok" value="수정" />
				    <input class="btn btn_info"  type="button" id="btn-delete" value="삭제" />
</c:if>
<c:if test="${viewMap.REPT_APRV_NO eq loginVO.id && viewMap.APRV_LINE_CD eq '3' }">
					<input class="btn btn_info" type="button" id="btn-ok" value="재결재" />
</c:if>
				    <input class="btn btn_info full-right"  type="button" id="btn-cancel" value="목록" />
				</div>
				</article>
			</section>
		</div>
