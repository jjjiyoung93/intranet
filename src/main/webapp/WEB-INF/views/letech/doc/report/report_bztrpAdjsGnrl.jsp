...
function fn_addBztrpItem(data) {
	for(key in data) {
		...
		if(data[key].BZTRP_ITEM_DIV == "CD0011001") {
			bztrpCtnt = data[key].BZTRP_ITEM_ST_PLC_NM + " → " + data[key].BZTRP_ITEM_ED_PLC_NM
		} else {
			bztrpCtnt = data[key].BZTRP_ITEM_CTNT
		}
		html += '	<td colspan="5" class="td-comm td-s5">' + bztrpCtnt + '</td>';
...
$(function() {
	...
	$("#corp_crd_use_amt").text(addCommas($("#corp_crd_use_amt").text()) + "원");
