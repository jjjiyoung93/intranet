...
function fn_changeBztrpItem(itemDiv) {
	...
	// 개별 영역
	if(itemVar == "CD0011001") { // 교통비
		// 내역 교통비 양식으로 변경
		$(cntn).find("input, select").remove();
		var html = '<select id="bztrp_item_st_plc_' + seq + '" name="bztrp_item_st_plc_' + seq + '" class="form-control bztrp_item_st_plcs" style="width:45%; display:inline;" onChange="fn_changePlc(this)">';
		html += 	'<option value="" selected disabled>--출발지--</option>';
		plcCodeList.forEach(item => {
			html += '	<option value="' + item.CD + '">' + item.CD_NM + '</option>';
		});
		html += '</select>';
		html += '<select id="bztrp_item_ed_plc_' + seq + '" name="bztrp_item_ed_plc_' + seq + '" class="form-control bztrp_item_ed_plcs" style="width:45%; display:inline; float:right;" onChange="fn_changePlc(this)">';
		html += 	'<option value="" selected disabled>--도착지--</option>';
		plcCodeList.forEach(item => {
			html += '	<option value="' + item.CD + '">' + item.CD_NM + '</option>';
		});
		html += '</select>';
		$(cntn).append(html);
		// 금액 readonly 추가, 초기화
		$(amt).find("input").attr("readonly", true);
		$(amt).find("input").val("");
		// 비고 초기화
		$(rmrk).find("input").val("");
	} else if(itemVar == "CD0011002") { // 여비
		// 내역 input 추가
		$(cntn).find("input, select").remove();
		var html = '<input id="bztrp_item_ctnt_' + seq + '" name="bztrp_item_ctnt_' + seq + '" type="text" class="form-control bztrp_item_ctnts" readonly>';
		$(cntn).append(html);
		// 금액 readonly 추가, 초기화
		$(amt).find("input").attr("readonly", true);
		$(amt).find("input").val("");
		...
		$(".bztrp_item_divs").each(function(i, v) {
			if($(v).val() == "CD0011002") { // 여비
				count++;
			}
		});
		if(count > 1) {
			$(itemDiv).val("");
			alert("여비는 한번만 입력할 수 있습니다.");
			return;
		}
		...
		// 내역 input 추가
		$(cntn).find("input, select").remove();
		var html = '<input id="bztrp_item_ctnt_' + seq + '" name="bztrp_item_ctnt_' + seq + '" type="text" class="form-control bztrp_item_ctnts">';
		$(cntn).append(html);
		// 금액 readonly 추가, 초기화
		$(amt).find("input").attr("readonly", false);
		$(amt).find("input").val("");
		// 비고 초기화
		$(rmrk).find("input").val("");
	} 
...
function fn_addBztrpItem() {
	...
	html += '		</select>';
	html += '	</td>';
	html += '	<td class="form-group"><input id="bztrp_item_ctnt_' + bztrpCount + '" name="bztrp_item_ctnt_' + bztrpCount + '" type="text" class="form-control bztrp_item_ctnts" readonly></td>';
	html += '	<td class="form-group"><input id="bztrp_item_amt_' + bztrpCount + '" name="bztrp_item_amt_' + bztrpCount + '" type="text" class="form-control bztrp_item_amts text-right convNum" readonly></td>';
	html += '	<td class="form-group"><input id="bztrp_item_rmrk_' + bztrpCount + '" name="bztrp_item_rmrk_' + bztrpCount + '" type="text" class="form-control bztrp_item_rmrks" readonly></td>';
	...
	$.ajax({
		...
		success: function(data2){
			...
			for(key in data) {
				...
				html += '	</td>';
				/* 내역 */
				if(bztrpDivCd == 'CD0011001') { // 일반-교통비
					html += '<td class="form-group">';
					...
					html += '	</select>';
				} else if(bztrpDivCd == 'CD0011002') { // 일반-여비
				...
				/* 금액 */
				html += '	<td class="form-group"><input id="bztrp_item_amt_' + bztrpSeq + '" name="bztrp_item_amt_' + bztrpSeq + '" type="text" class="form-control bztrp_item_amts text-right convNum" value="' + (data[key].BZTRP_ITEM_AMT == null ? "" : addCommas(data[key].BZTRP_ITEM_AMT)) + '원" ' + (bztrpDivCd == "CD0011001" || bztrpDivCd == "CD0011002" ? "readonly" : "") + '></td>';
				/* 비고 */
...
function getValidation2(valid) {
	...
	$(".tr_bztrp_items").each(function(i, obj) {
		...
		if($(obj).find(".bztrp_item_st_plcs option:selected").val() == "") {
			console.log($(obj).find(".bztrp_item_st_plcs option:selected").val() == "");
			if(valid) {
